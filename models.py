...
class VolunteerApplication(db.Model):
    ...
    status = db.Column(db.String(20), default='pending')  # pending, approved, rejected, trained
...
class TrainingParticipant(db.Model):
    ...
    firefighter_id = db.Column(db.Integer, db.ForeignKey('firefighter.id'), nullable=True)
    status = db.Column(db.String(20), default='registered')  # registered, attended, absent, completed
